  ...
  doMvAnimate () {
    ...
    this.gsap.to('.main-visual__image', {
      ...
      onComplete: async () => {
        ...
        await awatiFunction(function () {
          const navMenu = $('.nav-menu__button')
          $(navMenu).removeClass('nav-menu__button')
          $(navMenu).addClass('nav-menu__button-in-contents')
        }, 0)
      ...
      scrollTrigger: {
        ...
        onEnterBack: async (self) => {
          ...
          const spansKey = Object.keys(spans)
          await awatiFunction(function () {
            const navMenu = $('.nav-menu__button-in-contents')
            $(navMenu).removeClass('nav-menu__button-in-contents')
            $(navMenu).addClass('nav-menu__button')
          }, 0)
          await awatiFunction(function () {
            const mySkilsIcons = $('.sub-visual__my-skils-icons')
