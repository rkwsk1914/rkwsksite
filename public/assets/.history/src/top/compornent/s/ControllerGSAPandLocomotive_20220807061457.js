  ...
  init () {
    ...
    */
    ...
    navLinkLogo.addEventListener('click', async function (e) {
      ...
      await awatiFunction(this.cotrolleMvScrollBack.bind(this), 0)
      await awatiFunction(
        this.controllerNavMenu._closeMenu.bind(this),
        0,
        'nav-menu-button, nav-menu-button--in-contents'
      )
    }.bind(this))
    ...
    navLink.addEventListener('click', async function (e) {
      ...
      if (found) {
        ...
        switch (found.input) {
          case '#':
            await awatiFunction(this.cotrolleMvScrollBack.bind(this), 0)
            await awatiFunction(this.controllerNavMenu._closeMenu.bind(this), 0, 'nav-menu-button, nav-menu-button--in-contents')
            return
          case '#section-works':
