...
class DomAnimate {
  ...
  showTitle (elemClassName) {
    const span = document.getElementsByClassName(`${elemClassName} delay${index}`)
  ...
  doFadeInDown (elemClassName, isOut) {
    ...
    if (isOut === true) {
      ...
      $(elem).outClass('animate__fadeInUp')
      return
    }
  ...
  doFadeInUp (elemClassName, isOut) {
    ...
    if (isOut === true) {
      ...
      $(elem).outClass('animate__fadeOutDown')
      return
    }
...
export class ControllerGSAPandLocomotive extends DomAnimate {
  constructor () {
  ...
  doMvAnimate () {
    ...
    this.gsap.to('.main-visual__image', {
      ...
      onComplete: async () => {
        ...
        await awatiFunction(this.doFadeInDown.bind(this), 0)
        await awatiFunction(this.doFadeInDown, 0, 'nav-menu')
        for (let index = 1; index <= spansKey.length; index++) {
          await awatiFunction(this.showTitle, 50, 'sub-visual__title__strong__span')
        }
        await awatiFunction(this.doFadeInUp, 50, 'sub-visual__my-skils-icons')
      },
