...
export class ControllerGSAPandLocomotive {
  ...
  init () {
    ...
    $('.nav-menu__link').bind('click', async (e) => {
      ...
      if (found) {
        const backup_welcome = this.welcome
        const backup_intoWorks = this.intoWorks
        const backup_intoProfile = this.intoProfile
        this.welcome = false
        this.intoWorks = false
        this.intoProfile = false
        console.log(this.intoProfile)

        await awatiFunction(function () {
          switch (found.input) {
            case '#':
              cotrolleMvScrollBack()
              return
            case '#section-works':
              cotrolleWorkScroll()
              return
        ...
        this.welcome = backup_welcome
        this.intoWorks = backup_intoWorks
        this.intoProfile = backup_intoProfile
      }
    })
  }
